...
// debugOut writes the CLI output to {tempDir}/{stamp}-{name}-output.json,
// pretty-printed when it's valid JSON and raw otherwise.
func debugOut(tempDir, stamp, name, out string) {
	cliexec.DebugMaybeJSON(tempDir, stamp+"-"+cliexec.DebugName(name)+"-output.json", out)
}
