...
// uniform, greppable taxonomy — one line each, all prefixed [claude]:
// [thinking] / [text] (trimmed, truncated), [tool] [<Name>] <summary>, [tool]
// [denied] <content> for a rejected tool call, [system] (init collapsed to a
// marker, other subtypes passed through raw), and [<type>] / [<type>] [<sub>]
// for anything unrecognised. Malformed lines are skipped silently.
func runStream(cmd *exec.Cmd) (string, error) {
	var lastResultLine string

	err := cliexec.Stream(cmd, func(line string) {
		handleStreamLine(line, &lastResultLine)
	})
	if err != nil {
		return "", err
	}
...
// handleStreamLine parses one stream-json line, logs it under the [claude]
// taxonomy, and captures the CLI's result blob into *lastResult. A malformed
...
func handleStreamLine(line string, lastResult *string) {
	...
	switch event.Type {
	case "assistant":
		printAssistantEvent(event)
	case "result":
		*lastResult = line
	case "user":
		printUserEvent(event)
	case "system":
		printSystemEvent(event, line)
	default:
		...
		if event.Subtype != "" {
			cliexec.Logf("[claude] [%s] [%s]", event.Type, event.Subtype)
		} else {
			cliexec.Logf("[claude] [%s]", event.Type)
		}
	}
}
...
func printSystemEvent(event streamEvent, raw string) {
	if event.Subtype == "init" {
		cliexec.Logf("[claude] [system] init")
		return
	}
	cliexec.Logf("[claude] [system] %s", raw)
}
...
func printUserEvent(event streamEvent) {
	var msg userMessage
	...
	for _, block := range msg.Content {
		if block.Type == "tool_result" && block.IsError {
			...
			cliexec.Logf("[claude] [tool] [denied] %s", cliexec.Truncate(strings.TrimSpace(block.Content), 200))
		}
	}
}
...
func printAssistantEvent(event streamEvent) {
	...
	for _, block := range msg.Content {
		...
		case "thinking":
			...
			if trimmed != "" {
				cliexec.Logf("[claude] [thinking] %s", cliexec.Truncate(trimmed, 200))
			}
		case "text":
			trimmed := strings.TrimSpace(block.Text)
			if trimmed != "" {
				cliexec.Logf("[claude] [text] %s", cliexec.Truncate(trimmed, 200))
			}
		case "tool_use":
			if summary := formatToolInput(block.Name, block.Input); summary != "" {
				cliexec.Logf("[claude] [tool] [%s] %s", block.Name, summary)
				continue
			}
			...
			cliexec.Logf("[claude] [tool] [%s] %s", block.Name, cliexec.Truncate(string(text), 200))
		}
	}
}
